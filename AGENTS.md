...
- **Phase 17 M1 DONE (2026-08-18): StatCan metadata repository** (`docs/metadata-repo-plan.md`) — ingest the RDC non-confidential documentation corpus (2.4 GB zip in `docs/metadatarepo/`, **gitignored, never commit**) into a searchable longitudinal question bank feeding the hub's existing Searcher tile. Measured by the shipped classifier: 3,006 files / 319 groups / 1981–2026 / **1,919 data dictionaries** (the pre-build filename survey undercounted at ~1,193) yielding **~185k variable occurrences** (both languages; EN/FR pairing is M3). Key decisions: occurrences vs. concepts modelled separately; search moves server-side to Supabase Postgres FTS (client-side TF-IDF stays for bundled instruments); records project onto the existing `RegistryEntry` so designer insertion + DDI/JSON-LD export work unchanged. Licence settled: **Statistics Canada Open Licence** (attribution + no-endorsement + identify-as-adaptation are M5 acceptance criteria).
  - **M1 DONE (2026-08-18):** `packages/statcan-corpus` — Node-only ETL (zip reader, classifier, pdfjs extractor with measured row reconstruction, ingest, reporter, CLI). 297 tests. Classifies all 3,006 files in 4.9 s, **96.8% typed**, 1,919 data dictionaries; extraction 0 failures at ~1.8 docs/s. **D5: ~185k dictionary records ≈ 193–209 MB — fits Supabase's 500 MB free tier for M1–M3, but that figure excludes the pg_trgm index, the M3 concepts table and M4 content, so treat ~300 MB on a real loaded table as the trigger to move bulky payloads to Storage. Re-measure with `pg_total_relation_size` before M4.** Run: `pnpm --filter @mobilesurvey/statcan-corpus corpus:inventory`. Committed artifact: `docs/statcan-corpus-report.md` (byte-identical per run). **Not yet done:** real per-variant parsers (M2), so parsing is still prototype-grade; adversarial accuracy review of classification (96.8% is coverage, not accuracy).
