...
- **Phase 17 PLANNED (2026-08-18): StatCan metadata repository** (`docs/metadata-repo-plan.md`) — ingest the RDC non-confidential documentation corpus (2.4 GB zip in `docs/metadatarepo/`, **gitignored, never commit**) into a searchable longitudinal question bank feeding the hub's existing Searcher tile. Measured: 3,006 files / 318 surveys / 1980–2026 / ~99% text-extractable; `T15.2`+`T15.6` data dictionaries (1,193 files) yield an estimated ~177k variable definitions (~88.5k after EN/FR pairing). Key decisions: occurrences vs. concepts modelled separately; search moves server-side to Supabase Postgres FTS (client-side TF-IDF stays for bundled instruments); records project onto the existing `RegistryEntry` so designer insertion + DDI/JSON-LD export work unchanged. **Public deployment blocked on confirming the licence** (non-confidential is settled; republication terms are not).
