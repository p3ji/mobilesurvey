...
- **Phase 16 PLANNED (2026-07-17): Sensor module** — GPS-tagged collection + camera capture with ML-assisted coding (nutritional studies). Full architecture in `docs/sensor-module-plan.md`: two new response domains (`geolocation`, `photo`), instrument-level `SensorConfig` + runtime-enforced consent stored as routable `CONSENT_*` variables, `SensorServices` integration interface (mocked like the rest), private Supabase Storage bucket with client-side EXIF strip, pluggable `RecognitionProvider` (demo = Anthropic vision via the Validator-V3 key pattern). Phases S1–S4; nothing built yet.
