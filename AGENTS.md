...
- **(2026-06-29, PARTIALLY RESOLVED — Playwright QA)** A11y headings. Runtime survey pages had **no `<h1>`** (title was a `<span>`) — fixed: the title is now an `<h1 class="eq__title">` with `font-size:inherit; margin:0` so the visual is unchanged; runtime now passes the heading heuristic. Still open: designer Pro emits `<h3>` (inspector) before `<h2>` (preview) due to cross-panel DOM order — deferred (authoring tool, needs a panel heading-hierarchy pass). Full axe-core + manual audit still pending (Phase 9). Labels/aria/roles otherwise solid.
