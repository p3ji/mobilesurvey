...
- **Phase 15 SCOPED (2026-07-09, not started):** DDI-Lifecycle 3.3 compliance. Full design in `docs/ddi-compliance-plan.md`. Driven by an expert standards reviewer at the target org; their tool (https://github.com/pbisson44/ddigraph, ingests Lifecycle-as-FragmentInstance and does XML→RDF itself) is the acceptance target — so schema-valid FragmentInstance XML, not a JSON-LD exporter. Key known defects in `packages/ddi-xml`: unnamespaced root element, non-canonical URNs (`...!localId`), agency parsed from the instrument id, dots in item IDs likely illegal. Plan: P1 vendor XSDs + `xmllint-wasm` CI gate then fix exporter; P2 identity model (`agencyId` config, default placeholder `io.github.p3ji` — **never hardcode `ca.statcan`**, we don't act for the agency); P3 FragmentInstance packaging + ddigraph interop harness; P4 import ddigraph's real 14.5 MB Ireland-LFS fixtures.
