...
- **Phase 17 — StatCan metadata repository: M1–M2 done, M3/M4 MVP shipped (2026-08-19)** (`docs/metadata-repo-plan.md`) — the RDC non-confidential documentation corpus (2.4 GB zip in `docs/metadatarepo/`, **gitignored, never commit**) is now searchable end to end: archive → classify → extract → parse → Supabase → ranked search in the hub's Searcher. **1,368 dictionary PDFs → 436,962 variable occurrences + 850,912 response categories, in 24 min.** Occurrences and concepts stay modelled separately; records project onto the existing `RegistryEntry`, so designer insertion and the DDI/JSON-LD export chain work unchanged. Licence: **Statistics Canada Open Licence** — attribution, no-endorsement and identify-as-adaptation are live in the UI, carried by the single `CORPUS_ATTRIBUTION` string.
  ...
  - **M3 concept clustering DONE (2026-08-19), modelled on DDI’s variable cascade** (`sql/clusters.sql`, `src/cluster.ts`): `c:Concept` → `l:ConceptualVariable` (Concept + Universe) → `l:RepresentedVariable` (+ a coding) → the occurrence. Live: **70,750 concepts / 85,363 conceptual variables / 94,901 represented variables / 177,470 memberships**, 13,466 spanning >1 year. Keeping DDI’s three levels rather than one cluster is what makes “which cycles changed the coding” a *property* (`representations > 1`) instead of a pairwise comparison — and it finds 13,466 cross-cycle groups against 4,759 for survey+variable-name matching, 9,841 of them cross-survey. Hub tab: **Concepts over time**, with a timeline flagging where a series breaks.
  - **Source documents DONE (2026-08-19):** every record links to the dictionary it came from, opened at the cited page. Linking out to StatCan was measured and rejected — only **26 of 581 documents (4.5%)** carry an SDDS number, and those resolve to a survey page, not the dictionary. So `corpus_document` (DDI `r:OtherMaterial` + `r:Citation`) holds identity in Postgres and the reconstructed text goes to the **`corpus-documents` Storage bucket** — a separate 1 GB quota. Live: **581 documents · 113,678 pages · 223 MB · 1,477 chunks · ~257 MB of Storage**. Chunked at 100 pages so a citation costs ~180 KB even against the 3,567-page dictionary. Setup: **DEPLOYMENT.md §9f**.
  - **Typo-tolerant search DONE (2026-08-19):** `opiod` returned nothing while `opioid` returned 96 — FTS matches lexemes exactly. A **15,294-word vocabulary** (`corpus_term`, 298 KB, trigram-indexed) now corrects queries; the search auto-applies a correction, says so, and offers the original back. Ranking is `similarity × (1 + ln(records))`, because similarity alone corrects `maritial`→`martial` (6 recs, not 459) and `diabetis`→`diabetic` (48, not 380). **Feed it prose only** — variable names and the routing conditions in `universe` fragment into `opi`/`smk`, and `opi` outranked `opioids` for `opiod` until tokens with digits or underscores were dropped whole.
  - **WISHLISTED (decision 2026-08-19): French.** Not blocked by the free tier — blocked by nothing, deferred by choice. Measured: `corpus_variable` is 1,114.7 B/row of which **420.3 (37.7%) is derived** (`fts` 239.3, `search_text` 181.0), so slimming those two saves ~148 MB at both-language scale, more than French costs (~488 MB as-is vs ~340 MB slimmed, against a shared 500 MB). Order when picked up: slim storage → load French → EN↔FR pairing (pairing is pointless before the load; its job is letting a French record inherit its English cluster). Full arithmetic in the plan’s “Deferred to a second iteration”.
  - **NOT DONE / open:** the remaining **184 barren documents** (not one problem — `IMDB_BDIM`’s 52 are annexes with no variable entries, `CCR_RCC`’s 15 use a wrapped two-row header, census files bury entries in 1,753 pages; separating “will never yield records” from “layout not yet written” beats writing another parser); French category-row recall; `corpus_cluster_stats.coding_changed` counts over all CVs (7,850) not those spanning >1 year (2,807); designer insertion of a corpus record not exercised; `.doc`/`.docx` (M4, 604 files).
