...
Committed artifact: `docs/statcan-corpus-report.md`, regenerated byte-identically on every run.
...
**Verdict: the full corpus fits Supabase's 500 MB free tier at roughly 40% of the cap**, leaving
headroom for the existing app tables. The plan's Storage fallback (searchable projection in
Postgres, bulky payloads out) is **not needed** and should not be built.
...
- No adversarial review of classification accuracy has run — the workflow phase that would have
  hand-checked a 60-file sample was cut short. Treat 96.8% as *coverage*, not *accuracy*.
