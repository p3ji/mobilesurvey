...
/**
 * Dotted-numeric version compare (`'1.10.0'` > `'1.9.0'`): negative / zero / positive like a
 * comparator. Seeding uses this to refresh a stored bundled-survey row when the shipped bundle
 * is newer — bump the instrument's `version` whenever its content changes, or the update will
 * never reach the backend (upserts otherwise ignore existing rows).
 */
export function compareInstrumentVersions(a: string, b: string): number {
